...
class Example(ttk.LabelFrame):
    ...
    def bldOptionMenu(self):
        ...
        optFrm = LabelFrame(self.parent, text = 'OptionMenu')
        optFrm.pack(side = 'bottom', fill = 'both', expand = 1)
        
        self.ovar = tk.StringVar()
        options = ["OpenBSD", "NetBSD", "FreeBSD"]
        om = ttk.OptionMenu(optFrm , self.ovar , options[0], *options , command=self.onOptSelect)
        om.pack(side=LEFT , padx=15, fill = 'both', expand = 1)

        self.lvar = tk.StringVar()
        self.lvar.set("OpenBSD")
        lbl = Label(optFrm , textvariable=self.lvar)
        lbl.pack(side=LEFT)
        ttk.Separator(optFrm, orient = 'horizontal').pack(side='bottom', anchor = 'w', fill = 'both', expand = 1)
